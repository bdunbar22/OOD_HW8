...
for representing and editing cs3500.music. It allows for a number of operations to be made to a musical
work.

In this model cs3500.music is represented as a list of notes. Each note is an object with a pitch, start
...
according to the pitch enum and octave class as explained next.

The following pitches are supported: C C♯ D D♯ E F F♯ G G♯ A A♯ B
This model supports octaves -9 to 99. I made this decision based on the display of cs3500.music having 5
...
interfere with the display of cs3500.music, and will further allow for cs3500.music to be played in circumstances
where the cs3500.music maker may want to go outside of the range of human hearing, for dogs as an example.
...
against in the isPersisten function, the note can return if it is continuing to be played at the
starting beat of the check note and includes a check on the pitch and tone. This makes it possible
to display cs3500.music in a logical fashion. Finally, edits can be made to notes and fields can be
...
be used to sort notes by octave then pitch to find the tone range in a piece of cs3500.music.

Creating cs3500.music from notes is enabled with the INoteList and IPiece interfaces.
Creating cs3500.music from notes is realized with the NoteList and Piece classes.
...
This is a safe design and highly desired in the way I modeled cs3500.music because I want classes that
...
- change a note
- remove a note
- determine if the cs3500.music contains a note (using overridden equals of Note class)
- Get the notes in a list of cs3500.music (Returns a deep copied list for security)
- Get the number of the last beat that cs3500.music will be playing in.
  and
- DISPLAY MUSIC TO THE CONSOLE: String musicOutput();

From IPiece
(All methods return a new piece of cs3500.music so the original(s) aren't lost.)
- merge two pieces of cs3500.music in a serial fashion: One plays after the other.
- merge two pieces of cs3500.music in a parallel fashion: They will play at the same time.
...
- reverse a piece of cs3500.music
- copy a piece of cs3500.music
